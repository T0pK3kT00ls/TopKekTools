...
class WhatsUpServer(threading.Thread):
    ...
    def login(self):
        ...
        if self.ip not in accounts:
            ...
            while 1:
                ...
                if name in messages:
                    self.print_indicator( bcolors.OKBLUE +
            ...
            messages[name] = []
            self.print_indicator( bcolors.ENDC +
    ...
    def logoff(self):
        global clients
        global onlines
        self.conn.send( bcolors.ENDC + '## Bye!\n')
...
def main():
    ...
    sock.bind((HOST, PORT))
    sock.listen(5)
    print bcolors.BOLD + '-= TopKekServer =-'
    print bcolors.HEADER + '>> Listening on:', PORT
    print ''
